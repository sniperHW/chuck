...
static chk_event_loop *g_event_loop = NULL;
...
struct fiber_socket *new_fiber_socket(chk_stream_socket *_s) {

}


fiber_socket_t fiber_socket_connect(chk_sockaddr *remote) {
	fiber_socket_t       _socket;
	chk_fiber_t          _current;
	chk_stream_socket   *_s = NULL;
	int32_t              fd;
	struct fiber_socket *_fiber_socket;
	...
	_current = chk_current_fiber();

	if(!chk_is_vaild_fiber()) {
		return _socket;
	}	

    fd = socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
    if(0 != chk_connect(fd,remote,NULL,loop,connect_callback,&_current,-1)) {
    	return _socket;
    }

    chk_fiber_block(0,(void**)&_s);

    _fiber_socket = new_fiber_socket(_s);

    if(_fiber_socket) {
    	_socket = chk_get_refhandle((chk_refobj*)_fiber_socket);
    }
...
chk_acceptor *fiber_socket_listen(const char *ip,int16_t port) {
	...
	acceptor = chk_listen_tcp_ip4(g_event_loop,ip,port,chk_acceptor_cb,void *ud);
