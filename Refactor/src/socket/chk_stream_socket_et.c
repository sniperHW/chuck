...
//准备缓冲用于发起写请求
static inline int32_t prepare_send(chk_stream_socket *s,uint32_t *send_size) {
	int32_t          i = 0;
	chk_bytebuffer  *b = cast(chk_bytebuffer*,chk_list_begin(&s->send_list));
	chk_bytechunk   *chunk;
	uint32_t    datasize,size,pos;
	*send_size = 0;
...
//准备缓冲用于发起读请求
static inline int32_t prepare_recv(chk_stream_socket *s,uint32_t *recv_size) {
	chk_bytechunk  *chunk;
	int32_t         i = 0;
	uint32_t        buff_size,pos;
	*recv_size      = 0;
	if(!s->next_recv_buf) {
	...
	for(pos = s->next_recv_pos,chunk = s->next_recv_buf;;) {
		buff_size = chunk->cap - pos;
		s->wrecvbuf[i].iov_len  = buff_size;
		s->wrecvbuf[i].iov_base = chunk->data + pos;
		*recv_size += buff_size;
		++i;
		if(buff_size != s->option.recv_buffer_size) {
			pos = 0;
			if(!chunk->next) 
